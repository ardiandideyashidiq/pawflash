...
pub(crate) fn is_winusb_compatible(driver: impl AsRef<OsStr>) -> bool {
    let d = driver.as_ref().to_string_lossy();
...
pub(crate) fn get_winusb_device_path(dev: DevInst) -> Result<WCString, Error> {
    ...
    if let Some(reg_key) = dev.registry_key() {
        let guid = reg_key
            .query_value_guid("DeviceInterfaceGUIDs")
            .or_else(|_| reg_key.query_value_guid("DeviceInterfaceGUID"));
        if let Ok(guid) = guid {
            let paths = dev.interfaces(guid);
            if let Some(path) = paths.iter().next() {
                return Ok(path.to_owned());
            }
        }
...
pub(crate) fn get_usbccgp_winusb_device_path(child: DevInst) -> Result<WCString, Error> {
    ...
    if !is_winusb_compatible(&driver) {
        debug!("Incompatible driver {driver:?} for interface, not WinUSB compatible");
        return Err(Error::new(
            ErrorKind::Unsupported,
