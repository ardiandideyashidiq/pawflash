...
export const ForceFastbootDialog = memo(function ForceFastbootDialog({
  ...
  const waitingTitle =
    stage === "sending"
      ? "Sending FASTBOOT handshake..."
      : stage === "confirmed"
        ? "Fastboot detected..."
        : "Waiting for preloader...";
  const waitingBody =
    stage === "sending"
      ? "Handshake sent to the MediaTek preloader. Waiting for the device to re-enumerate."
      : stage === "confirmed"
        ? "Fastboot interface detected. Finalizing session."
        : "Listening for the MediaTek preloader serial port.";
          ...
          <div className="space-y-3 px-4 py-4">
            <div className="status-shell flex items-center gap-3 px-3 py-3">
              {phase === "complete" ? (
                ...
                <XCircle
                  className={cn("h-5 w-5 shrink-0", phase === "cancelled" ? "text-warning" : "text-error")}
                />
              ) : (
                <LoaderCircle className="h-5 w-5 shrink-0 animate-spin text-trace-copper" />
              )}
